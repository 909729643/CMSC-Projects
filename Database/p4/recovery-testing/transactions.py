...
class LockTable:
	...
	@staticmethod
	def visit(G, u, color, found_cycle):
		color[u] = "gray"
		for v in G[u]:
			if color[v] == "gray":
				found_cycle.append(v)
			if color[v] == "white":
				LockTable.visit(G,v,color,found_cycle)
		color[u]="black"
	...
	def detectDeadlocksAndChooseTransactionsToAbort():
		...
		with LockTable.hashtable_lock:
			for entry in LockTable.lockhashtable:
				hold_list = LockTable.lockhashtable[entry].current_transactions_and_locks
				wait_list = LockTable.lockhashtable[entry].waiting_transactions_and_locks
				for (wid,wtype) in wait_list:
					if wid not in graph:
						graph[wid] = []
					for (hid,htype) in hold_list:
						graph[wid].append(hid)
						if not graph.has_key(hid):
							graph[hid] = []
		...
		for u in graph:
			if color[u] == "white":
				LockTable.visit(graph,u,color,found_cycle)
...
class LogManager:
	...
	def restartRecovery():
		...
		for item in allrecords:
			if item.info[1] == LogRecord.COMMIT and item.info[0] not in redolist:
				redolist.append(item.info[0])
		#undo
		for item in allrecords:
			if item.info[1] == LogRecord.START and item.info[0] not in undolist:
				undolist.append(item.info[0])
			if item.info[1] == LogRecord.ABORT and item.info[0] in undolist:
				undolist.remove(item.info[0])
			if item.info[1] == LogRecord.COMMIT and item.info[0] in undolist:
				undolist.remove(item.info[0])
		for item in reversed(undolist):
			LogManager.revertChanges(item)
			LogManager.createAbortLogRecord(item)
		for item in redolist:
			redorecords = [lr for lr in allrecords if lr.info[0] == item]
			for lr in redorecords:
				if lr.info[1] == LogRecord.UPDATE:
					...
					relation_name = Relation.getRelationByName(lr.info[2])
		BufferPool.writeAllToDisk(relation_name)
