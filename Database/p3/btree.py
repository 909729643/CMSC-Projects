...
class BTreeBlock(Block):
    ...
    def redistributeWithBlock(self, otherBlock):				
		...
		if otherBlock.isUnderfull():#if (N' is a predecessor of N) self:N' other:N
			if not self.isLeaf:
				...
				for i in range(0,len(parentBlock.keysAndPointers),2):
					if parentBlock.keysAndPointers[i].blockNumber == self.blockNumber:
						kprime = i+1 # Find the key in the parent between the pointers to self and otherBlock
				pm = self.keysAndPointers.pop(len(self.keysAndPointers) - 1)
				km1 = self.keysAndPointers.pop(len(self.keysAndPointers) - 1)
				otherBlock.keysAndPointers.insert(0,parentBlock.keysAndPointers[kprime])
				otherBlock.keysAndPointers.insert(0,pm)
				otherBlock.keysAndPointers[0].getBlock().parent = Pointer(otherBlock.blockNumber)
				return km1
			else:
				km = self.keysAndPointers.pop(len(self.keysAndPointers) - 2)
				pm = self.keysAndPointers.pop(len(self.keysAndPointers) - 2)
				otherBlock.keysAndPointers.insert(0,km)
				otherBlock.keysAndPointers.insert(0,pm)
				return km
		else: # self:N other:N'
			if not self.isLeaf:
				...
				for i in range(0,len(parentBlock.keysAndPointers),2):
					if parentBlock.keysAndPointers[i].blockNumber == self.blockNumber:
						kprime = i+1
				p1 = otherBlock.keysAndPointers.pop(0)
				k1 = otherBlock.keysAndPointers.pop(0)
				self.keysAndPointers.append(parentBlock.keysAndPointers[kprime])
				self.keysAndPointers.append(p1)
				self.keysAndPointers[len(self.keysAndPointers)-1].getBlock().parent = Pointer(self.blockNumber)
				return k1
			else:
				p1 = otherBlock.keysAndPointers.pop(0)
				k1 = otherBlock.keysAndPointers.pop(0)
				k2 = otherBlock.keysAndPointers[1]
				self.keysAndPointers.insert(len(self.keysAndPointers) - 1,p1)
				self.keysAndPointers.insert(len(self.keysAndPointers) - 1,k1)
				return k2
